...
int main()
{
    ...
    while (1)
    {
        ...
        if (ch != '\r' && ch != '\n')
        {
            // Maka menyimpan karakter ke inputWords[index]
            inputWords[index] = ch;
    ...
    */
    ...
    char lines[255];
    index = 0;
    //melakukan set ulang index ke 0 untuk mengembalikan posisi pointer ke awal
    char *pecahan = strtok(inputWords, ",");
    //menyimpan nilai dari pecahan char yang dipisahan oleh koma
    while (pecahan != NULL)
    {
        //jika nilai pecahan != null maka nilai akan disimpan pada variabel lines
        //lalu program akan memperbaharui nilai pada variabel pecahan
        lines[index] = pecahan[0];
        lines[++index] = '\0';
        //selanjutnya jika tidak ditemukan char "," pada pecahan maka variabel pecahan
        //diisi dengan NULL dan pengulangan akan di hentikan
        pecahan = strtok(NULL, ",");
    }

    bubbleSort(lines);
    //untuk mengurutkan nilai pada variabel lines 
